...
module.exports = function (app) {
  ...
  app.post("/home/search", async function (req, res, next) {
    ...
    try {
      ...
      await page.goto("https://www.indeed.com/jobs?q=" + keyword + "&l=" + location)
      ...
      if (html) {
        ...
        $(".tapItem.fs-unmask").each(function (i, element) {
          var data = {
            id: $(element).find(".jcs-JobTitle").attr("data-jk"),
            title: $(element)
          ...
          $($(element).find(".job-snippet >ul >li")).each(function (
            i,
            element
          ) {
            var line = $(element)
  ...
  app.post("/home/search/showmore", async function (req, res, next) {
    ...
    try {
      ...
      const page = await browser.newPage()
      page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/113.0.0.0 Safari/537.36')
      await page.goto("https://www.indeed.com/jobs?q=" +
        keyword +
        "&l=" +
        location +
        "&start=" +
